TERMUX_PKG_HOMEPAGE=https://github.com/axel-download-accelerator/axel
TERMUX_PKG_DESCRIPTION="light command line download accelerator"
TERMUX_PKG_LICENSE="GPL-2.0"
TERMUX_PKG_MAINTAINER="@termux"
TERMUX_PKG_VERSION=2.17.11
TERMUX_PKG_REVISION=2
TERMUX_PKG_SRCURL=https://github.91chi.fun/https://github.com/axel-download-accelerator/axel/releases/download/v${TERMUX_PKG_VERSION}/axel-${TERMUX_PKG_VERSION}.tar.gz
TERMUX_PKG_SHA256=f12682f981c83f374b82b0668f25f694a2042b617b236ee6fb057e5473c54e31
TERMUX_PKG_AUTO_UPDATE=true
TERMUX_PKG_DEPENDS="openssl"
